...
uint16_t u16CalCrc(uint8_t *pu8Data , uint16_t u16DataLen)
{
	...
	while(u16DataLen--) 
	{
		...
		u8CrcLo = u8CrcHi ^ u8CRCHi[u8Index] ;
		u8CrcHi = u8CRCLo[u8Index] ;
	};
	
	return (u8CrcHi * 0x100 + u8CrcLo) ;
}
...
void USART_SendStr(uint32_t USARTx, uint8_t *str, uint16_t str_len)
{
    ...
    for(i = 0; i < str_len; i++)
    {
        usart_data_transmit(USARTx, str[i]);
...
{
    uint16_t u16Temp=__REV16(*((uint16_t*)(pU8_Ptr)));
    return u16Temp;
}

uint32_t u32ConvBtoL(uint8_t* pU8_Ptr)
{
    uint32_t u32Temp=__REV(*((uint32_t*)(pU8_Ptr)));
    return u32Temp;
}
